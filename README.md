...
- В Docker-режиме ссылки автоматически используют публичный Cloudflare Quick Tunnel URL. Для своего домена задайте `PUBLIC_WEBHOOK_BASE_URL`.
...
- По умолчанию он поднимает три контейнера: приложение, PostgreSQL 16 и бесплатный Cloudflare Quick Tunnel.
- Приложение по умолчанию стартует с `DB_BACKEND=postgres`, а строка подключения берётся из `DATABASE_URL`.
- Приложение само получает публичный `trycloudflare.com` URL и ставит Telegram webhook. VK Callback server создаётся автоматически, если указан `VK_GROUP_ID`.
...
1. Скопируйте `.env.example` в `.env` и заполните минимум: `TELEGRAM_BOT_TOKEN`, `VK_GROUP_TOKEN`, `VK_CONFIRMATION_TOKEN`, `PROXYAPI_URL`, `PROXYAPI_KEY`, `ADMIN_IDS`, `ADMIN_TOKEN`.
...
8. Проверьте логи `docker compose logs -f proxyapi-bots`: там должен появиться публичный URL и статус установки webhook.
...
## Локальный туннель
- По умолчанию используется Cloudflare Quick Tunnel через контейнер `proxyapi-cloudflared`, без токенов и платных доменов.
- Если хотите использовать свой домен или другой tunnel, задайте `PUBLIC_WEBHOOK_BASE_URL=https://your-domain.example` в `.env`.
